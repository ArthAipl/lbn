...
class CustomAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  Size get preferredSize => Size.fromHeight(
    kToolbarHeight + ((bottom is PreferredSizeWidget) ? (bottom as PreferredSizeWidget).preferredSize.height : 0),
  );
}
...
class _OneToOnePageState extends State<OneToOnePage>
  ...
  Future<void> _loadUserData() async {
    ...
    if (storedGropCode != null && storedGID != null) {
      // Get user MID from members API
      try {
        final response = await http.get(Uri.parse('https://tagai.caxis.ca/public/api/member'));
        if (response.statusCode == 200) {
          final jsonData = json.decode(response.body);
          List<dynamic> data = jsonData is Map<String, dynamic> && jsonData.containsKey('members')
              ? jsonData['members']
              : [];
          ...
          final userMember = data
              ...
              .firstWhere(
                (member) => member.email == storedEmail && member.gropCode == storedGropCode,
                orElse: () => Member(
                  id: 0,
                  name: '',
                  email: '',
      ...
      if (response.statusCode == 200) {
        ...
        try {
          final membersList = data
              .map((json) => Member.fromJson(json as Map<String, dynamic>))
              .where((member) =>
                  member.gropCode == widget.gropCode &&
                  member.status == 1 &&
                  member.email != widget.loggedInUserEmail)
              .toList();
  ...
  Widget _buildMemberCard(Member member) {
    return Card(
      ...
      child: InkWell(
        ...
        onTap: () {
          if (widget.loggedInUserMID != null && widget.loggedInGID != null) {
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => ArrangeMeetingPage(
                  member: member,
                  fromMID: widget.loggedInUserMID,
                  gId: widget.loggedInGID,
                ),
              ),
            );
          } else {
